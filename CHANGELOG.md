...
- Tocados: `commands/*` (14 renombres a `brain-*`), `skills/brain-coach/*`, `install.sh`, `scripts/sync.py`, `motor-code/actualizar/{SKILL.md,check-update.sh}`, `kit/brain/CLAUDE.md`, `uninstall.sh`, `README.md`, `README.en.md`, `VERSION`, `.claude-plugin/{plugin,marketplace}.json`, `CHANGELOG.md`.

## 2.31.1 (2026-06-29)
- **Fix: instrucción de instalación en Cowork (estaba mal).** El README/install.sh decían `/setup-cowork install brain@agentic-second-brain`, pero ese flujo solo instala del registry oficial curado de Anthropic, no de un marketplace propio (confirmado por test real en Cowork). El camino correcto, sin terminal, para un usuario individual es: **Customize → Plugins → Add marketplace → `brunogiel/agentic-second-brain` → instalar `brain`** (los repos públicos sí se soportan para "Add marketplace" individual; la limitación "solo privados" aplica únicamente a organization marketplaces). Corregido en `README.md`, `README.en.md` y el header de `install.sh`. De paso, agregada la alternativa por plugins en Claude Code (`/plugin marketplace add` + `/plugin install`) junto al curl. (Publicar al community marketplace de Anthropic queda como opción futura, solo para descubribilidad.)
