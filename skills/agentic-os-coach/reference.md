<!--
  REFERENCE — el manual que crece (Personal Agentic OS).
...
Esto no se lee de un tirón. Es una caja de herramientas: venís acá cuando querés subir un escalón. Tu `CLAUDE.md` raíz alcanza para arrancar. Esto es para cuando quieras crecer. ¿No sabés por dónde seguir? Pedile **`/agentic-os-coach`**.
...
1. **[DET]** Leer la versión publicada del kit (el archivo `VERSION` del repo) y compararla con la instalada (`~/.claude/skills/agentic-os-coach/VERSION`).
...
name: actualizar
description: >
  Chequea si el kit Personal Agentic OS tiene una versión nueva y, si vos querés, la instala.
  Usalo cuando digas "¿hay updates?", "actualizá el kit", "fijate si salió algo nuevo",
  "actualizá Personal Agentic OS".
---

# actualizar: bajar las mejoras del kit

## Flujo
1. [DET] Leé la versión publicada (VERSION del repo) y la instalada (~/.claude/skills/agentic-os-coach/VERSION).
...
Tu `/agentic-os-coach` es un orquestador chiquito: no hace tu sistema, te lleva de la mano para que lo armes vos.
...
Y si el desarrollo es en serio (epics, PRDs, un equipo), eso es un deporte aparte: hay métodos dedicados al ciclo de desarrollo agéntico, como [BMAD-METHOD](https://github.com/bmad-code-org/BMAD-METHOD). No compiten con esto: Personal Agentic OS se queda con tu contexto y decisiones; el método de dev maneja la construcción dentro del repo. Conviven en este mismo split.
...
¿No sabés en qué escalón estás? Pedile **`/agentic-os-coach`**.
