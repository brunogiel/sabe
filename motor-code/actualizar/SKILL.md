...
description: >
  ...
  "actualizá Personal Agentic OS", "¿estoy al día?".
---

# actualizar: bajar las mejoras del kit

## Qué es
Mira si el kit (Personal Agentic OS) tiene una versión más nueva que la tuya y, con tu OK, la baja.
...
1. **[DET]** Comparar versiones. En Claude Code corré el script `check-update.sh` de esta carpeta;
   ...
   (`https://raw.githubusercontent.com/brunogiel/personal-agentic-os/main/VERSION`) y compará con `~/.claude/skills/agentic-os-coach/VERSION`.
...
3. **[DET]** Con el OK: corré el `install.sh` del repo (vuelve a bajar el motor y los archivos del
   kit; respeta lo tuyo, no pisa tus entregables). El `install.sh` ya refresca `~/.claude/skills/agentic-os-coach/VERSION`
...
## Qué lee y escribe
- **Lee:** `~/.claude/skills/agentic-os-coach/VERSION` (local) y el `VERSION` / `CHANGELOG.md` del repo.
- **Escribe:** solo con tu OK, baja archivos del kit y actualiza `~/.claude/skills/agentic-os-coach/VERSION`. Nunca toca tus
...
- Si actualizó, `~/.claude/skills/agentic-os-coach/VERSION` quedó en el número nuevo.
